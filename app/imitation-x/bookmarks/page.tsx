...
async function getBookmarkedFeed(userId: string, search: string, limit: number = 20): Promise<FeedPage> {
...
export default async function BookmarksPage({
  ...
  return (
    <div>
      <ArrowLeftBack name={tSidebar('bookmarks')} />
      ...
      {!user?.id ? (
        ...
        <div className='px-4 pt-4'>
          <BookmarksContent userId={user.id} search={search} />
        </div>
      )}
    </div>
  )
